...
membersRouter.post("/", (req, res) => {
  const { name, email, member_type, level } = req.body as {
    name: string; email?: string; member_type?: "male" | "female" | "guest"; level?: number;
  };
  if (!name?.trim()) { res.status(400).json({ message: "Name is required" }); return; }
  const id = uuid();
  db.prepare(
    "INSERT INTO members (id, club_id, name, email, member_type, level) VALUES (?, ?, ?, ?, ?, ?)"
  ).run(id, clubId(req), name.trim(), email ?? null, member_type ?? "male", level ?? 2);
  const member = db.prepare("SELECT * FROM members WHERE id = ?").get(id);
  res.status(201).json({ member });
});

membersRouter.patch("/:id", (req, res) => {
  const { name, member_type, level } = req.body as { name?: string; member_type?: "male" | "female" | "guest"; level?: number };
  const { id } = req.params;
  ...
  if (level !== undefined) db.prepare("UPDATE members SET level = ? WHERE id = ? AND club_id = ?").run(level, id, clubId(req));
